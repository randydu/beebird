    ...
    def setJobClass(cls, clsJob):
        if cls._clsJob_ is not None:
            ...
            if cls._clsJob != job._CallableTaskJob:
                ...
                raise Exception(
                    f"task ({cls.__name__}) is already binded to job class {cls._clsJob_.__name__}")
        cls._clsJob_ = clsJob
...
def _task_func(func, public):
    ...
    WrapTask = type(tskName, (Task,), {
                    **fields, **{'__init__': init, '__doc__': func.__doc__}})

    if public:
        TaskMan.instance().registerTask(json_serialize(
            WrapTask))  # pylint: disable= no-member
